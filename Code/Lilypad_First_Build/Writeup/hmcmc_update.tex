...
\begin{enumerate}
\item Drum-up some good set of interpolating points $S$, calculate and $f(S) = {V(s) : s \in S}$  using the log-posterior density $V(q) = ln(P[q|\vec{y}])$ where for a posterior dist $P$ from data $\vec{y}$ over parameter vector $q$, using $q$ to reflect that this is also the generalized position vector.
\item Following Wendland (2007, ``Computational Aspects of Radial Basis Function Approximation'') set the
...
\item \textit{Sample}
\begun{enumerate}
\item Starting at point $x$ we propose a point $x'$ by numerically integrating a Hamiltonian system on the approximated potential energy surface $V(q) = -\text{ln}(\hat{\pi}(q)$ with a random initial momentumand quadratic kinetic energy (ie. $K(p) = p^T M P$ where $M$ is a mass matrix).
